...
- 仓库与发布链路在关键口径可核验，但本地已超前 release：
  ...
  - npm：`0.18.3`
  - 最新 release：`v0.18.3`
  - 本地 main 比 release 领先：`30` 个提交（建议下个版本打 tag 并按 `npm publish` 同步）
...
- `work/openai-readiness-20260603-final-livecheck.md`：已生成（当前口径）
- `work/external-evidence-20260603-final-livecheck.md`：已生成（当前口径）
- `work/metrics-20260603-final-livecheck.md`：已生成（当前口径）
- `npm`, GitHub 发布口径验证通过
