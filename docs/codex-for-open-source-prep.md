...
截至 2026-06-01，`ai-devtools-cn` GitHub 最新 release 是 `v0.17.1`，npm 已发布版本仍是 `ai-devtools-cn@0.16.1`：

- GitHub stars：3
- Forks：1
- Merged PRs：116
- Closed issues：107
- Open issues：7（包含 npm 0.17.1 发布同步 #223 和 #45-#49 外部贡献任务）
- Releases：21
...
```text
This public OSS project provides Chinese AI developer workflow templates for PR review, issue triage, test generation, documentation, releases, and tool evaluation. It helps developers and maintainers adopt AI tools safely and consistently. The project has 116 merged PRs, 107 closed issues, 21 releases, CI, a published npm CLI, and one external feedback issue.
```
...
当前 npm 公开包仍是 `0.16.1`，GitHub `main` 已准备 `0.17.1` 发布边界。`pr-pack`、`recipes` 等 npx 路径需要等 #223 完成并验证后再对外使用。
