...
在仓库本地，如果你要快速查看本发布包、反馈入口和外部 PR 邀请入口，可以运行：
...
npm 包与发布口径已对齐（截至 2026-06-03，`npm` / `package.json` / `release` 均为 `0.18.3`）。若你发现网络或环境差异，请以 `npm view ai-devtools-cn version` 与 `npm run templates:publish-status` 的双核验为准。当前可用 clone + `npm run templates:launch` 作为保底。
...
- npm 当前可见版本：`0.18.3`（按 `npm view ai-devtools-cn version` 核验）
...
```text
当前 npm 公开版本已与 GitHub main 对齐（版本 `0.18.3`）。如用户本地环境有缓存差异，建议先用 clone 路径验证：
...
npx ai-devtools-cn@latest recipes
npx ai-devtools-cn@latest recipes pr-review-docs
npx ai-devtools-cn@latest trial --template pr-review --output work/trial

如果你在排查 CI，可以改成：

npx ai-devtools-cn@latest recipes ci-failure
npx ai-devtools-cn@latest trial --template ci-troubleshooting --scenario "debug a failing CI job" --output work/trial-ci

试完后请提交一条公开安全反馈：
https://github.com/ONEISALL7/ai-devtools-cn/issues/new?template=template_feedback.yml

如果你是按 30 分钟 pilot 任务包试用，请用这个结构化表单：
https://github.com/ONEISALL7/ai-devtools-cn/issues/new?template=external_pilot_feedback.yml

不要提交 token、客户信息、内部日志、未公开源码或个人隐私。
```

如果你要主动邀请 2-3 位真实外部开发者完成 30 分钟 pilot，可以先生成可复制邀请和跟进表：
...
这份邀请包会给出一对一文案、公开渠道文案、推荐 pilot recipe 和维护者跟进表。它不会自动发送消息；外部反馈仍必须来自真实用户自己的 GitHub issue 或 PR。
...
推荐先让对方选择 #45-#49 中的一个 issue，并在 issue 下留言认领。然后运行：
...
现在有模板、CLI 和真实案例；npm 包与发布口径已对齐到 `0.18.3`，可对外建议使用 `npx ai-devtools-cn@latest` 进行体验。
