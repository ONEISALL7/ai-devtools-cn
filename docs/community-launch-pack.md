...
- npm 当前发布版本：`0.16.1`
- GitHub 最新 release：`v0.17.1`
- 外部反馈：已有 1 条公开 feedback issue，并已转化为 pnpm workspace CI 试用包
- 外部 merged PR：目前仍为 0
- npm 0.17.1 发布同步：见 [#223](https://github.com/ONEISALL7/ai-devtools-cn/issues/223)
...
```text
当前 npm 公开版本仍是 0.16.1，npm 0.17.1 发布前请先 clone 仓库试用：
...
当前 npm 公开版本仍是 0.16.1，GitHub 最新 release 是 0.17.1。npm 0.17.1 发布前，请优先使用 clone 仓库路径。
...
0.17.1 发布到 npm 并验证后，才把上面的命令换成 npx 路径：
...
当前 npm 包还是 0.16.1，npm 0.17.1 发布前先 clone 仓库试用：
...
当前 npm 包还是 0.16.1，npm 0.17.1 发布前建议先 clone 仓库试用：
