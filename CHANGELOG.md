...
- 刷新 OpenAI 申请所需的证据链路快照脚本与文档，让可核验指标、外部反馈和 readiness 材料一键复现。
- 将 evidence/readiness 输出调整为发布页友好的链接格式，减少人工核验误差。
- 修正本地发布状态中的 markdownlint 与链接稳健性，保留 `npm`/`release`/`external feedback` 的可直接核验输出。

## 0.18.3 - 2026-06-02

- 移除公开仓库中的非项目使用文档和生成命令入口，避免项目首页和文档暴露与项目使用价值无关的准备材料
- 新增公开安全脱敏模板，帮助用户在提交 issue、PR、日志、反馈或案例前检查敏感信息并生成可公开文本
- 更正公开发布说明中的 npm 同步状态，明确 npm latest 仍落后于 GitHub main，外部用户在 `0.18.3` 发布前应优先使用 clone + `npm run templates:*`
...
- 新增 External pilot feedback issue 模板，结构化记录外部试用者身份、试用配方、运行命令、真实场景、公开证据链接和安全边界
- 更新 `pilot` 任务包反馈入口，让外部试用用户优先提交 `external_pilot_feedback.yml`
